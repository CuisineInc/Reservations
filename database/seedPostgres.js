...
const bookingsGenerator = function () {
  ...
  while (counter <= 500000){
    ...
    let randomUserIndex = getRandomInt(userArr.length);
    // create a property called user_id and set it to userArray[randomUserIndex]
    bookingObj.user_id =  userArr[randomUserIndex]
...
const insertRestaurantTable = function (){
  for (let i = 0; i < restArr.length; i++){
    let query = `INSERT INTO restaurants(id, restaurant_name, seat_capacity) VALUES(${restArr[i].id}, '${restArr[i].restaurant_name}', ${restArr[i].seat_capacity})`
    console.log(i)
...
const insertUsersTable = function (){
  for (let i = 0; i < userArr.length; i++){
    let query = `INSERT INTO users(id, first_name, last_name) VALUES(${userArr[i].id}, '${userArr[i].first_name}', '${userArr[i].last_name}')`
    console.log(i)
    client.query(query, (err, res) => {
      console.log(err, res)
    })
...
const insertTablesTable = function (){
  for (let i = 0; i < restArr.length; i++){
    let query = `INSERT INTO tables(id, table_size, restaurant_id) VALUES(${tablesArr[i].id}, ${tablesArr[i].table_size}, ${tablesArr[i].restaurant_id})`
    console.log(i)
    client.query(query, (err, res) => {
      console.log(err, res)
      if (i === restArr.length){
...
const insertBookingsTable = function (){
  for (let i = 0; i < bookingsArr.length; i++){
    let query = `INSERT INTO bookings(id, date_time, party_size, restaurant_id, users_id, tables_id) VALUES(${bookingsArr[i].id}, '${bookingsArr[i].date_time}', ${bookingsArr[i].party_size}, ${bookingsArr[i].restaurant_id}, ${bookingsArr[i].users_id}, ${bookingsArr[i].tables_id})`
    console.log(i)
    client.query(query, (err, res) => {
      console.log(err, res)
    })
